...
const storedData = window.localStorage.getItem('bamData');
...
useEffect(() => {
  ...
  window.localStorage.setItem('bamData', JSON.stringify(dataToStore));
...
async function removeItem(partName) {
  ...
  const q = query(collection(db, "bamData"), where("name", "==", partName));
  ...
  await deleteDoc(doc(db, "bam-closet", docId));
 ...
 async function changeQuan(id) {
  ...
  console.log("Id:", id);
  await updateDoc(doc(db, 'bam-closet', id), {
...
function nameSubmit() {
  let id;
  addDoc(collection(db, "bam-closet"), {
    name: name,
    quan: quan,
    location: location
  }).then(docRef => {
...
var updatedPartsList = partsList 
  ...
  async function refresh() {
    ...
    const qDcaf = query(collection(db, "bam-closet"))
